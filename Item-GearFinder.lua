...
local function make_button(object)
	...
	button.loadguide = CHAIN(CreateFrame("Button", nil, button, nil))
		...
		:SetScript("OnClick",function(self,b) 
			if button.dungeonguide then
				if not ZGV.Tabs:TryToActivateGuide(button.dungeonguide) then 
					local tab = ZGV.Tabs:GetTabFromPool()
					tab:SetAsCurrent()
					tab:AssignGuide(button.dungeonguide.title,button.dungeonguide.CurrentStepNum or 1)
					tab:ActivateGuide()
				end
			end
		end)
	...
	MF.AddButton = CHAIN(CreateFrame("Button", nil, MF.CenterColumn, nil))
		...
		:SetScript("OnClick",function(self,button) 
			if GearFinder.BestDungeonGuide then
				if not ZGV.Tabs:TryToActivateGuide(GearFinder.BestDungeonGuide) then 
					local tab = ZGV.Tabs:GetTabFromPool()
					tab:SetAsCurrent()
					tab:AssignGuide(GearFinder.BestDungeonGuide.title,GearFinder.BestDungeonGuide.CurrentStepNum or 1)
					tab:ActivateGuide()
				end
			end
		end)
