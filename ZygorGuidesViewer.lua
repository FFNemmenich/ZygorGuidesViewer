...
function ZGV.InPhase2(phasename)
	if not phasename then return true end
	if phasename==ZGV.db.profile.fakephase then return true end

	local hasbuff = ZGV.Parser.ConditionEnv.hasbuff2

	phasename = phasename:lower():gsub(" ","")

	if phasename=="olddarnassus" then
		return hasbuff("spell:277430") or hasbuff("spell:277427") or UnitLevel('player')<110
	elseif phasename=="oldundercity" then
		return hasbuff("spell:276824") or hasbuff("spell:276827") or not (PlayerCompletedQuest(51795) or PlayerCompletedQuest(51796))
	elseif phasename=="oldsilithus" then
		return hasbuff("spell:264606") or hasbuff("spell:262016") or hasbuff("spell:255152") or UnitLevel('player')<110
	elseif phasename=="oldblastedlands" then
		return hasbuff("spell:176111") or UnitLevel('player')<90
	elseif phasename=="newblastedlands" then
		return not (hasbuff("spell:176111") or UnitLevel('player')<90)
	elseif phasename=="bfa" then
		local qa=ZGV.questsbyid[46728]
		local qh=ZGV.questsbyid[50769]
		return (PlayerCompletedQuest(46728) or qa and qa.inlog) or (PlayerCompletedQuest(50769) or qh and qh.inlog)
	...
	elseif phasename=="newarathi" then
		return C_Map.GetMapArtID(14)==1137
	-- just in case: spellids for arathi are 276954 276950
	...
	elseif phasename=="oldpeak" then
		return hasbuff("spell:276827") or hasbuff("spell:276824")
	end
end

function ZGV:TestPhases()
	local phases = {"olddarnassus","oldundercity","oldsilithus","oldblastedlands","bfa","oldarathi","newarathi","newarathicontrolled","oldpeak"}
	for i,ph in ipairs(phases) do
		print(ph,":",ZGV.InPhase2(ph) and "|cff00ff00YES" or "|cffff0000NO")
	end
end
